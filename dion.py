...
class Dion(Optimizer):
    ...
    def _init_opt_state_dion(
        ...
        if isinstance(param, DTensor):
            ...
            Q = torch.randn(Q_shape, device=param.device, dtype=param.dtype)
...
def dion_update(
    ...
    if not transpose:
        ...
        X.addmm_(Q, P.T, alpha=-scaled_lr)
...
def power_iteration(
    ...
    """
    Returns a low-rank approximation of U by power iteration.
    Compute P and Q such that (approximately) U ≈ P @ Q^T.
    """
    Q = Q_init
...
def orthogonalize_dtensor(
    ...
    if m <= r:
        ...
        replicate = [Replicate() for _ in range(P.device_mesh.ndim)]
        P_local = P.redistribute(placements=replicate).to_local()
        Q_local, _ = torch.linalg.qr(P_local)
    ...
    # Randomized Cholesky QR
        ...
        # Calculate right triangular matrix R using standard QR, and solve for Q
        replicate = [Replicate() for _ in range(P.device_mesh.ndim)]
        P_local = P.redistribute(placements=placements).to_local()
        SP_local = SP.redistribute(placements=replicate).to_local()
        _, R_local = torch.linalg.qr(SP_local, mode="r")
        Q_local = torch.linalg.solve_triangular(
            R_local, P_local, upper=True, left=False
        )
        Q = DTensor.from_local(
            Q_local,
            device_mesh=P.device_mesh,
        ...
        # Apply another iteration of Cholesky QR to better orthogonalize Q
        QQ: DTensor = Q.T @ Q
        QQ_local = QQ.redistribute(placements=replicate).to_local()
        R_local, _ = torch.linalg.cholesky_ex(QQ_local, upper=True)
        Q_local = torch.linalg.solve_triangular(
            R_local, Q_local, upper=True, left=False
        )
        Q = DTensor.from_local(
            Q_local,
            device_mesh=P.device_mesh,
