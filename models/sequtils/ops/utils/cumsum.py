...
def chunk_global_cumsum_scalar_kernel(
    ...
    i_b, i_h = i_bh // H, i_bh % H
    if USE_OFFSETS:
        bos, eos = tl.load(offsets + i_b).to(tl.int32), tl.load(offsets + i_b + 1).to(
            tl.int32
        )
    else:
        bos, eos = i_b * T, i_b * T + T
    ...
    if offsets is None:
        ...
        if indices is None:
            indices = torch.cat(
                [
                    torch.stack(
                        [offsets.new_full((n,), i), offsets.new_tensor(range(n))], 1
                    )
                    for i, n in enumerate(
                        triton.cdiv(offsets[1:] - offsets[:-1], BT).tolist()
                    )
                ]
            )
        NT = len(indices)
...
def chunk_global_cumsum_scalar(
    ...
    chunk_global_cumsum_scalar_kernel[grid](
        s, z, offsets, T=T, H=H, HEAD_FIRST=head_first, REVERSE=reverse
    )
    return z
...
def chunk_global_cumsum_vector(
    ...
    chunk_global_cumsum_vector_kernel[grid](
        s, z, offsets, T=T, H=H, S=S, BS=BS, HEAD_FIRST=head_first, REVERSE=reverse
    )
    return z
...
def chunk_global_cumsum(
    ...
    if offsets is not None:
        assert (
            s.shape[0] == 1
        ), "Only batch size 1 is supported when offsets are provided"
    if len(s.shape) == 3:
        return chunk_global_cumsum_scalar(
            s, dtype, reverse, offsets, head_first, output_dtype
        )
    elif len(s.shape) == 4:
...
def chunk_local_cumsum(
    ...
    if len(g.shape) == 3:
        return chunk_local_cumsum_scalar(
            g, chunk_size, reverse, offsets, indices, head_first, output_dtype
        )
    elif len(g.shape) == 4:
