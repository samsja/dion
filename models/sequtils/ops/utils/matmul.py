...
@triton.autotune(
    configs=[
        ...
        triton.Config(
            {"BM": 128, "BK": 128, "BN": 256, "G": 4}, num_stages=3, num_warps=8
        ),
        triton.Config(
            {"BM": 256, "BK": 128, "BN": 128, "G": 4}, num_stages=3, num_warps=8
        ),
        triton.Config(
            {"BM": 256, "BK": 128, "BN": 64, "G": 4}, num_stages=4, num_warps=4
        ),
        triton.Config(
            {"BM": 64, "BK": 128, "BN": 256, "G": 4}, num_stages=4, num_warps=4
        ),
        triton.Config(
            {"BM": 128, "BK": 128, "BN": 128, "G": 4}, num_stages=4, num_warps=4
        ),
        triton.Config(
            {"BM": 128, "BK": 64, "BN": 64, "G": 4}, num_stages=4, num_warps=4
        ),
        triton.Config(
            {"BM": 64, "BK": 64, "BN": 128, "G": 4}, num_stages=4, num_warps=4
        ),
        triton.Config(
            {"BM": 128, "BK": 64, "BN": 32, "G": 4}, num_stages=4, num_warps=4
        ),
    ],
    key=["M", "N", "K"],
)
...
def matmul(a, b, activation=""):
    assert (
        a.shape[1] == b.shape[0]
    ), "Incompatible dimensions (A: {}x{}, B: {}x{})".format(*a.shape, *b.shape)
