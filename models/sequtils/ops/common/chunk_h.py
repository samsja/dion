    ...
    for i_t in range(NT):
        ...
        if HEAD_FIRST:
            p_k = tl.make_block_ptr(
                k + i_nh * T * K, (K, T), (1, K), (i_k * BK, i_t * BT), (BK, BT), (0, 1)
            )
            p_v = tl.make_block_ptr(
                v + i_nh * T * V, (T, V), (V, 1), (i_t * BT, i_v * BV), (BT, BV), (1, 0)
            )
        ...
        if USE_GK:
            if HEAD_FIRST:
                ...
                p_gk_last = (
                    gk + i_nh * T * K + last_idx * K + i_k * BK + tl.arange(0, BK)
                )
        ...
        if USE_GV:
            if HEAD_FIRST:
                ...
                p_gv_last = (
                    gv + i_nh * T * V + last_idx * V + i_v * BV + tl.arange(0, BV)
                )
    ...
    for i_t in range(NT - 1, -1, -1):
        ...
        if HEAD_FIRST:
            p_q = tl.make_block_ptr(
                q + i_nh * T * K, (K, T), (1, K), (i_k * BK, i_t * BT), (BK, BT), (0, 1)
            )
...
def chunk_fwd_h(
    ...
    BS = (
        ...
        else min(split_size, max(16, triton.next_power_of_2(T)))
    )
    assert (
        BS % BT == 0
    ...
    if head_first:
        h = k.new_empty(
            B, H, NS, K, V, dtype=k.dtype if not states_in_fp32 else torch.float
        )
    else:
        h = k.new_empty(
            B, NS, H, K, V, dtype=k.dtype if not states_in_fp32 else torch.float
        )
...
def chunk_bwd_dh(
    ...
    if head_first:
        dh = k.new_empty(
            B, HQ, NS, K, V, dtype=k.dtype if not states_in_fp32 else torch.float
        )
    else:
        dh = k.new_empty(
            B, NS, HQ, K, V, dtype=k.dtype if not states_in_fp32 else torch.float
        )
