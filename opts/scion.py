# Implementation from https://github.com/LIONS-EPFL/scion/blob/main/scion.py
...
class Sign(Norm):
    ...
    def lmo(self, g):
        if g.ndim == 1:  # Add compatibility for 1D tensors
            d_in = g.shape[0]
        else:
            d_out, d_in = g.shape
        if self.normalized:
    ...
    def init(self, w):
        if self.zero_init:
            ...
            if w.ndim == 1:  # Add compatibility for 1D tensors
                d_in = w.shape[0]
            else:
                d_out, d_in = w.shape
            w.data = (
